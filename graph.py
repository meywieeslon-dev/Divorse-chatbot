...
def synthesize_answer(state: DivorceState) -> dict:
    ...
    system_text = (
        ...
        "Отвечай ТОЛЬКО на вопросы про развод и семейное право: дети, "
        "алименты, раздел имущества, ЗАГС, суд, документы. "
        "Если последний вопрос пользователя не об этом (например, про "
        "животных, кулинарию, погоду, работу) — НЕ отвечай на него по "
        "существу и не давай никаких советов по той теме. Вместо этого "
        "одной короткой вежливой фразой скажи, что помогаешь только с "
        "вопросами о разводе, и предложи вернуться к теме. "
        "В остальных случаях дай общую справочную рекомендацию с учётом "
        "собранных фактов. "
